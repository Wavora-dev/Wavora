...
/**
 * Registers the "wavora://" custom URI protocol handler in Windows Registry
 * under HKEY_CURRENT_USER (no admin rights required).
 *
 * Registry structure:
 * ```
 * HKCU\Software\Classes\wavora
 *     (Default) = "URL:Wavora Protocol"
 *     URL Protocol = ""
 *     \DefaultIcon
 *         (Default) = "\"<exe_path>\",0"
 *     \shell\open\command
...
object WindowsProtocolRegistrar {
    ...
    private const val SCHEME = "wavora"
    ...
    fun register() {
        ...
        try {
            ...
            Logger.d(TAG, "Registering wavora:// protocol handler -> $exePath")

            // Main key with protocol description
            regAdd(REG_KEY, null, "URL:Wavora Protocol")
    ...
    private fun isAlreadyRegistered(currentExePath: String): Boolean {
        return try {
            ...
            // Registry stores path with quotes: "C:\path\to\Wavora.exe" "%1"
    ...
    private fun resolveExePath(): String? {
        ...
        //   <app>/Wavora.exe
        // So we go: java.home → parent (runtime) → parent (app) → Wavora.exe
        ...
        if (appDir != null) {
            val exeFile = java.io.File(appDir, "Wavora.exe")
            if (exeFile.exists()) {
